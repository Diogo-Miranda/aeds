...
void start(ListaDupla* listaDupla) 
{
	...
	// Cria cedula cabeça
	listaDupla->primeiro = novaCedula(placebo);
...
{
	// Cria nova cedula
	Cedula* nova = novaCedula(jogador);
...
void swap(ListaDupla* listaDupla, Jogador *maior, Jogador *menor) 
{
	// swap
	Jogador* aux;
	memcpy(aux, maior, sizeof(Jogador));
	memcpy(maior, menor, sizeof(Jogador));
	memcpy(menor, aux, sizeof(Jogador));
}

int getPosI(ListaDupla* listaDupla, Cedula* cedula) 
{
	int i = 0;
	for(Cedula* aux = listaDupla->primeiro->prox; aux->prox != cedula->prox; aux = aux->prox) i++;

	return i;
}

int getPosJ(ListaDupla* listaDupla, Cedula* cedula) 
{
	int j = 0;

	for(Cedula* aux = listaDupla->ultimo; aux->ant != cedula->ant; aux = aux->ant) j++;

	return j;
}

Jogador getPivo(ListaDupla* listaDupla, int i, int j) 
{
	int meio = ( (i+j) / 2);
	Cedula* aux = listaDupla->primeiro->prox;

	for(int k = i; k <= meio; k++) aux = aux->prox;
...
void quicksortRec(ListaDupla* listaDupla, Cedula* esq, Cedula* dir) 
{
	...
	int posI = getPosI(listaDupla, esq);
	int posJ = getPosJ(listaDupla, dir);

	Jogador pivo = getPivo(listaDupla, posI, posJ); 
	...
	while(i != j || i->ant != j) {
		...
		if (i != j || i->ant != j) {
			swap(listaDupla, &j->elemento, &i->elemento);
			i = i->prox; j = j->ant;
		}
	...
	if(j->prox != esq) quicksortRec(listaDupla, esq, j);
	if(dir != i) quicksortRec(listaDupla, i, dir);
}
