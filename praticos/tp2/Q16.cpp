...
bool comparar(Jogador *tmp, Jogador *atual) 
{
	...
	NUM_COMP++;	
	if(strcmp(getAnoNascimento(tmp), getAnoNascimento(atual)) == 0) {
		NUM_COMP++;
		out = strcmp(getNome(tmp), getNome(atual)) < 0;
	} else {
		NUM_COMP++;
		out = strcmp(getAnoNascimento(tmp), getAnoNascimento(atual)) < 0;
	}

	return out;
}

void insertionSortParcial(Jogador *jogador, int n, int K) 
{
	int i, j;
	Jogador tmp;
	for(i = 1; i < n; i++) {
...
int main(void)
{
    ...
    FILE *file = fopen("705657_insercaoParcial.txt", "w");
	
	int K = 10;
	
    double inicio = clock();	
	
	insertionSortParcial(jogador, qntJogadores, K);	
