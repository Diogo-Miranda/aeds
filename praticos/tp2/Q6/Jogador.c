...
void selectionSort_rec(Jogador *jogador, int qntJogadores, int i, int j, int menor)
{
    if(j < qntJogadores && i < qntJogadores-1) {
        NUM_COMP++;
        if( strcmp(getNome(&jogador[j]), getNome(&jogador[menor]) ) < 0)
            menor = j;
        selectionSort_rec(jogador, qntJogadores, i, j+1, menor);
    } else if (i < qntJogadores-1) {
        swap(jogador, i, menor);
        NUM_MOV += 3;
        selectionSort_rec(jogador, qntJogadores, i+1, i+2, i+1);
    }
}

void selectionSort(Jogador *jogador, int qntJogadores)
{
    selectionSort_rec(jogador, qntJogadores, 0, 1, 0);
}
...
int main(void)
{
	...
	FILE *file = fopen("705657_selecao.txt", "w");
	
	double inicio = clock();	
	
    selectionSort(jogador, qntJogadores);
